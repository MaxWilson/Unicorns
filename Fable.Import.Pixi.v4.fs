...
module PIXI =
    ...
    type [<Import("EventEmitter","pixi.js")>] EventEmitter() =
    ...
    and [<Import("DisplayObject","pixi.js")>] DisplayObject() =
    ...
    and [<Import("Container","pixi.js")>] Container() =
    ...
    and [<Import("GraphicsData","pixi.js")>] GraphicsData(lineWidth: float, lineColor: float, lineAlpha: float, fillColor: float, fillAlpha: float, fill: bool, shape: U4<Circle, Rectangle, Ellipse, Polygon>) =
    ...
    and [<Import("Graphics","pixi.js")>] Graphics() =
        inherit Container()
    ...
    and [<Import("Point","pixi.js")>] Point(?x: float, ?y: float) =
        member __.x with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
        member __.y with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    ...
    and [<Import("Matrix","pixi.js")>] Matrix() =
    ...
    and [<Import("Circle","pixi.js")>] Circle(?x: float, ?y: float, ?radius: float) =
    ...
    and [<Import("Ellipse","pixi.js")>] Ellipse(?x: float, ?y: float, ?width: float, ?height: float) =
        interface HitArea with
            member __.contains(x: float, y: float): bool = failwith "JS only"
        member __.x with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
        member __.y with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    ...
    and [<Import("Polygon","pixi.js")>] Polygon([<ParamArray>] points: float[]) =
        interface HitArea with
            member __.contains(x: float, y: float): bool = failwith "JS only"
    ...
    and [<Import("Rectangle","pixi.js")>] Rectangle(?x: float, ?y: float, ?width: float, ?height: float) =
        interface HitArea with
            member __.contains(x: float, y: float): bool = failwith "JS only"
        member __.x with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
        member __.y with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    ...
    and [<Import("RoundedRectangle","pixi.js")>] RoundedRectangle(?x: float, ?y: float, ?width: float, ?height: float, ?radius: float) =
        interface HitArea with
            member __.contains(x: float, y: float): bool = failwith "JS only"
        member __.x with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
        member __.y with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    ...
    and [<Import("ParticleContainer","pixi.js")>] ParticleContainer(?size: float, ?properties: ParticleContainerProperties list, ?batchSize: float) =
        inherit Container()
    ...
    and [<Import("SystemRenderer","pixi.js")>] SystemRenderer(system: string, ?width: float, ?height: float, ?options: RendererOptions list) =
        inherit EventEmitter()
    ...
    and [<Import("CanvasRenderer","pixi.js")>] CanvasRenderer(?width: float, ?height: float, ?options: RendererOptions list) =
    ...
    and [<Import("CanvasBuffer","pixi.js")>] CanvasBuffer(width: float, height: float) =
    ...
    and [<Import("CanvasGraphics","pixi.js")>] CanvasGraphics() =
    ...
    and [<Import("CanvasMaskManager","pixi.js")>] CanvasMaskManager() =
    ...
    and [<Import("CanvasTinter","pixi.js")>] CanvasTinter() =
    ...
    and [<Import("WebGLRenderer","pixi.js")>] WebGLRenderer(?width: float, ?height: float, ?options: RendererOptions list) =
        inherit SystemRenderer("")
    ...
    and [<Import("AbstractFilter","pixi.js")>] AbstractFilter(?vertexSrc: U2<string, ResizeArray<string>>, ?fragmentSrc: U2<string, ResizeArray<string>>, ?uniforms: obj) =
    ...
    and [<Import("SpriteMaskFilter","pixi.js")>] SpriteMaskFilter(sprite: Sprite) =
    ...
    and [<Import("BlendModeManager","pixi.js")>] BlendModeManager(renderer: WebGLRenderer) =
        inherit WebGLManager(unbox null)
        member __.setBlendMode(blendMode: float): bool = failwith "JS only"

    and [<Import("FilterManager","pixi.js")>] FilterManager(renderer: WebGLRenderer) =
        inherit WebGLManager(unbox null)
    ...
    and [<Import("MaskManager","pixi.js")>] MaskManager() =
        inherit WebGLManager(unbox null)
    ...
    and [<Import("ShaderManager","pixi.js")>] ShaderManager(renderer: WebGLRenderer) =
        inherit WebGLManager(unbox null)
    ...
    and [<Import("StencilManager","pixi.js")>] StencilManager(renderer: WebGLRenderer) =
        inherit WebGLManager(unbox null)
    ...
    and [<Import("WebGLManager","pixi.js")>] WebGLManager(renderer: WebGLRenderer) =
        member __.onContextChange with get(): Func<unit> = failwith "JS only" and set(v: Func<unit>): unit = failwith "JS only"
        member __.renderer with get(): WebGLRenderer = failwith "JS only" and set(v: WebGLRenderer): unit = failwith "JS only"
        member __.destroy(): unit = failwith "JS only"

    and [<Import("Shader","pixi.js")>] Shader(shaderManager: ShaderManager, vertexSrc: string, fragmentSrc: string, uniforms: obj, attributes: obj) =
    ...
    and [<Import("ComplexPrimitiveShader","pixi.js")>] ComplexPrimitiveShader(shaderManager: ShaderManager) =
        inherit Shader(unbox null, null, null, null, null)


    and [<Import("PrimitiveShader","pixi.js")>] PrimitiveShader(shaderManager: ShaderManager) =
        inherit Shader(unbox null, null, null, null, null)


    and [<Import("TextureShader","pixi.js")>] TextureShader(shaderManager: ShaderManager, ?vertexSrc: string, ?fragmentSrc: string, ?customUniforms: obj, ?customAttributes: obj) =
        inherit Shader(unbox null, null, null, null, null)
    ...
    and [<Import("ObjectRenderer","pixi.js")>] ObjectRenderer() =
        inherit WebGLManager(unbox null)
    ...
    and [<Import("RenderTarget","pixi.js")>] RenderTarget(gl: WebGLRenderingContext, width: float, height: float, scaleMode: float, resolution: float, root: bool) =
        member __.gl with get(): WebGLRenderingContext = failwith "JS only" and set(v: WebGLRenderingContext): unit = failwith "JS only"
    ...
    and [<Import("Sprite","pixi.js")>] Sprite(?texture: Texture) =
        inherit Container()
    ...
    and [<Import("SpriteRenderer","pixi.js")>] SpriteRenderer() =
    ...
    and [<Import("Text","pixi.js")>] Text(?text: string, ?style: TextStyle list, ?resolution: float) =
        inherit Sprite()
    ...
    and [<Import("BaseTexture","pixi.js")>] BaseTexture(source: U2<HTMLImageElement, HTMLCanvasElement>, ?scaleMode: float, ?resolution: float) =
        inherit EventEmitter()
    ...
    and [<Import("RenderTexture","pixi.js")>] RenderTexture(renderer: U2<CanvasRenderer, WebGLRenderer>, ?width: float, ?height: float, ?scaleMode: float, ?resolution: float) =
    ...
    and [<Import("Texture","pixi.js")>] Texture(baseTexture: BaseTexture, ?frame: Rectangle, ?crop: Rectangle, ?trim: Rectangle, ?rotate: float) =
    ...
    and [<Import("TextureUvs","pixi.js")>] TextureUvs() =
    ...
    and [<Import("VideoBaseTexture","pixi.js")>] VideoBaseTexture(source: HTMLVideoElement, ?scaleMode: float) =
        inherit BaseTexture(unbox null)
    ...
    and [<Import("utils","pixi.js")>] utils() =
    ...
    and [<Import("GroupD8","pixi.js")>] GroupD8 =
    ...
    module extras =
        ...
        and [<Import("extras.BitmapText","pixi.js")>] BitmapText(text: string, ?style: BitmapTextStyle list) =
            inherit Container()
        ...
        and [<Import("extras.MovieClip","pixi.js")>] MovieClip(textures: ResizeArray<Texture>) =
            inherit Sprite()
        ...
        and [<Import("extras.TilingSprite","pixi.js")>] TilingSprite(texture: Texture, width: float, height: float) =
            inherit Sprite()
    ...
    module filters =
        type [<Import("filters.AsciiFilter","pixi.js")>] AsciiFilter() =
            inherit AbstractFilter()
            member __.size with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"

        and [<Import("filters.BloomFilter","pixi.js")>] BloomFilter() =
            inherit AbstractFilter()
        ...
        and [<Import("filters.BlurFilter","pixi.js")>] BlurFilter() =
            inherit AbstractFilter()
        ...
        and [<Import("filters.BlurXFilter","pixi.js")>] BlurXFilter() =
            inherit AbstractFilter()
            member __.passes with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
            member __.strength with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
            member __.blur with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"

        and [<Import("filters.BlurYFilter","pixi.js")>] BlurYFilter() =
            inherit AbstractFilter()
            member __.passes with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
            member __.strength with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
            member __.blur with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"

        and [<Import("filters.SmartBlurFilter","pixi.js")>] SmartBlurFilter() =
            inherit AbstractFilter()


        and [<Import("filters.ColorMatrixFilter","pixi.js")>] ColorMatrixFilter() =
            inherit AbstractFilter()
        ...
        and [<Import("filters.ColorStepFilter","pixi.js")>] ColorStepFilter() =
            inherit AbstractFilter()
            member __.step with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"

        and [<Import("filters.ConvolutionFilter","pixi.js")>] ConvolutionFilter(matrix: ResizeArray<float>, width: float, height: float) =
            inherit AbstractFilter()
            member __.matrix with get(): ResizeArray<float> = failwith "JS only" and set(v: ResizeArray<float>): unit = failwith "JS only"
            member __.width with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
            member __.height with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"

        and [<Import("filters.CrossHatchFilter","pixi.js")>] CrossHatchFilter() =
            inherit AbstractFilter()


        and [<Import("filters.DisplacementFilter","pixi.js")>] DisplacementFilter(sprite: Sprite, ?scale: float) =
            inherit AbstractFilter()
            member __.map with get(): Texture = failwith "JS only" and set(v: Texture): unit = failwith "JS only"
            member __.scale with get(): Point = failwith "JS only" and set(v: Point): unit = failwith "JS only"

        and [<Import("filters.DotScreenFilter","pixi.js")>] DotScreenFilter() =
            inherit AbstractFilter()
        ...
        and [<Import("filters.BlurYTintFilter","pixi.js")>] BlurYTintFilter() =
            inherit AbstractFilter()
            member __.blur with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"

        and [<Import("filters.DropShadowFilter","pixi.js")>] DropShadowFilter() =
            inherit AbstractFilter()
            member __.blur with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
            member __.blurX with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
            member __.blurY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
        ...
        and [<Import("filters.SepiaFilter","pixi.js")>] SepiaFilter() =
            inherit AbstractFilter()
            member __.sepia with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"

        and [<Import("filters.ShockwaveFilter","pixi.js")>] ShockwaveFilter() =
            inherit AbstractFilter()
        ...
        and [<Import("filters.TiltShiftAxisFilter","pixi.js")>] TiltShiftAxisFilter() =
            inherit AbstractFilter()
            member __.blur with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
        ...
        and [<Import("filters.TiltShiftFilter","pixi.js")>] TiltShiftFilter() =
            inherit AbstractFilter()
            member __.blur with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
            member __.gradientBlur with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
            member __.start with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
        ...
        and [<Import("filters.TiltShiftXFilter","pixi.js")>] TiltShiftXFilter() =
            inherit AbstractFilter()
            member __.updateDelta(): unit = failwith "JS only"

        and [<Import("filters.TiltShiftYFilter","pixi.js")>] TiltShiftYFilter() =
            inherit AbstractFilter()
            member __.updateDelta(): unit = failwith "JS only"

        and [<Import("filters.TwistFilter","pixi.js")>] TwistFilter() =
            inherit AbstractFilter()
            member __.offset with get(): Point = failwith "JS only" and set(v: Point): unit = failwith "JS only"
            member __.radius with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
            member __.angle with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"

        and [<Import("filters.FXAAFilter","pixi.js")>] FXAAFilter() =
            inherit AbstractFilter()
    ...
    module interaction =
        type [<Import("interaction.InteractionData","pixi.js")>] InteractionData() =
        ...
        and [<Import("interaction.InteractionManager","pixi.js")>] InteractionManager(renderer: U2<CanvasRenderer, WebGLRenderer>, ?options: obj) =
    ...
    module loaders =
        ...
        and [<Import("loaders.Loader","pixi.js")>] Loader(?baseUrl: string, ?concurrency: float) =
        ...
        and [<Import("loaders.Resource","pixi.js")>] Resource(?name: string, ?url: U2<string, ResizeArray<string>>, ?options: LoaderOptions) =
            inherit EventEmitter()
    ...
    module mesh =
        type [<Import("mesh.Mesh","pixi.js")>] Mesh(texture: Texture, ?vertices: ResizeArray<float>, ?uvs: ResizeArray<float>, ?indices: ResizeArray<float>, ?drawMode: float) =
            inherit Container()
        ...
        and [<Import("mesh.Rope","pixi.js")>] Rope(texture: Texture, points: ResizeArray<Point>) =
        ...
        and [<Import("mesh.Plane","pixi.js")>] Plane(texture: Texture, ?segmentsX: float, ?segmentsY: float) =
            inherit Mesh(unbox null)
        ...
        and [<Import("mesh.MeshRenderer","pixi.js")>] MeshRenderer(renderer: WebGLRenderer) =
            inherit ObjectRenderer()
            member __.indices with get(): ResizeArray<float> = failwith "JS only" and set(v: ResizeArray<float>): unit = failwith "JS only"
    ...
    module ticker =
        type [<Import("ticker.Ticker","pixi.js")>] Ticker() =
